...
const renvoyerDemande = async (demande) => {
  try {
    ...
    if (response.data.success) {
      ...
      closeModal()
      showNotification('Demande renvoyée avec succès!', 'success')
    } else {
      showNotification(response.data.message || 'Erreur lors du renvoi de la demande', 'error')
    }
  } catch (error) {
    console.error('Erreur lors du renvoi de la demande:', error)
    showNotification('Erreur lors du renvoi de la demande', 'error')
  }
}

...
<head>
  ...
  <style>
    * {
    ...
    body {
      font-family: DejaVu Sans, DejaVu Sans Condensed, Arial, Helvetica, sans-serif;
      background: #fff;
      padding: 24px;
      color: #111;
    }
    ...
    /* Increased margin to push signature down */
    .releve-fait-a {
      margin-bottom: 50px;
      font-weight: bold;
    }
    ...
    @if ($type === 'AttestationReussite')
      <div class="reussite-container">
        ...
        <div class="avis-footer">
          Avis important: Il ne peut être délivré qu'un seul exemplaire de cette attestation. Aucun duplicata ne sera
          fourni.
        </div>
      </div>
    ...
    @elseif ($type === 'AttestationScolarite')
      <div class="scolarite-new">
        <table class="header-table">
          <tr>
            ...
            <td class="header-center-col">
              <img src="{{ public_path('images/logo_ensa.png') }}" alt="Logo"
                style="width: auto; height: 100px; display: block; margin: 0 auto;">
            </td>
        ...
        <div class="info-list">
          ...
          <div class="info-item">né le <strong>{{ $dateNaissance }}</strong> à
            <strong>{{ strtoupper($lieuNaissance) }}</strong></div>
          <div class="info-item">Poursuit ses études à l' Ecole Nationale des Sciences Appliquées Tétouan pour l'année
            universitaire <strong>{{ date('Y') }}/{{ date('Y') + 1 }}</strong>.</div>
          <br>
          <div class="info-item">Diplôme : <strong>Cycle Ingénieur</strong></div>
          <div class="info-item">Filière :
            <strong>{{ optional(optional($etudiant)->filiere)->nomF ?? 'Génie Informatique' }}</strong></div>
    ...
    @elseif ($type === 'ReleveNote')
      <div class="releve-new-container">
        ...
        <table style="width: 100%; margin-bottom: 10px;">
          <tr>
            <td style="text-align: center; font-size: 11px;">
              Année universitaire &nbsp; <strong>{{ optional($releve)->annee ?? date('Y') . '/' . (date('Y') + 1) }}</strong>
              &nbsp; <span class="releve-header-arabic">ﺔـﻴـﻌـﻣﺎـﺠـﻟا ﺔـﻨـﺴـﻟا</span>
            </td>
          </tr>
        </table>
        ...
        <table style="width: 100%; border-top: 1px solid #000; padding-top: 5px;">
          <tr>
            <td style="font-weight: bold; text-align: left; width: 50%;">Ecole Nationale des Sciences Appliquées Tétouan
            </td>
            ...
            @if (!empty($notes))
              @foreach ($notes as $n)
                <tr>
                  ...
                  <td style="text-align: center;">S1
                    {{ substr(optional($releve)->annee, 2, 2) }}/{{ substr(optional($releve)->annee, 7, 2) }}</td>
                </tr>
              @endforeach
            @else
              <tr>
        ...
        <div class="releve-footer-msg">
          Avis important : Il ne peut être délivré qu'un seul exemplaire du présent relevé de note. Aucun duplicata ne
          sera fourni.
        </div>
      </div>
    ...
    @elseif ($type === 'ConventionStage')
      <div class="convention-new">
        <table class="conv-header-table">
          <tr>
            ...
            <div style="text-align: center; margin: 0 20px;">
              <img src="{{ public_path('images/logo_ensa.png') }}" alt="Logo ENSA" style="width: auto; height: 100px;">
            </div>
        ...
        <div class="conv-party-block">
          <div class="conv-party-head">L'Ecole Nationale des Sciences Appliquées, Université Abdelmalek Essaâdi - Tétouan
          </div>
        ...
        <div class="conv-party-block">
          ...
          Représentée par Monsieur {{ optional($convention)->representantEntreprise ?? 'N/A' }} en qualité de
          {{ optional($convention)->fctRepresentant ?? 'N/A' }}
          <div class="conv-denom">Ci-après dénommée L'ENTREPRISE</div>
        </div>

        <div class="conv-art">
          <div class="conv-art-title">Article 1 : Engagement</div>
          L'ENTREPRISE accepte de recevoir à titre de stagiaire <strong>{{ strtoupper($nom) }}
            {{ strtoupper($prenom) }}</strong> étudiant de la filière du Cycle Ingénieur «
          {{ optional(optional($etudiant)->filiere)->nomF ?? 'N/A' }} » de l'ENSA de Tétouan, Université Abdelmalek
          Essaâdi (Tétouan), pour une période allant du
          <strong>{{ optional($convention)->dateDebut ? \Carbon\Carbon::parse($convention->dateDebut)->format('d/m/Y') : 'N/A' }}</strong>
          au
          <strong>{{ optional($convention)->dateFin ? \Carbon\Carbon::parse($convention->dateFin)->format('d/m/Y') : 'N/A' }}</strong><br>
          En aucun cas, cette convention ne pourra autoriser les étudiants à s'absenter durant la période des contrôles ou
          des enseignements.
        </div>

        <div class="conv-art">
          <div class="conv-art-title">Article 2 : Objet</div>
          Le stage aura pour objet essentiel d'assurer l'application pratique de l'enseignement donné par l'Etablissement,
          et ce, en organisant des visites sur les installations et en réalisant des études proposées par L'ENTREPRISE.
        </div>
        ...
        <div class="conv-art">
          ...
          L'ENTREPRISE/Stagiaire/Etablissement, L'ENTREPRISE désigne Mme/Mr
          <strong>{{ optional($convention)->encadrantEntreprise ?? 'N/A' }}</strong> encadrant(e) et parrain(e), pour
          superviser et assurer la qualité du travail fourni par le Stagiaire.<br>
          L'Etablissement désigne <strong>{{ optional($convention)->encadrantAcademique ?? 'N/A' }}</strong> en tant que
          tuteur qui procurera une assistance pédagogique.
        </div>
        ...
        <div class="conv-art">
          ...
          Le contenu de ce programme doit permettre au Stagiaire une réflexion en relation avec les enseignements ou le
          projet de fin d'études qui s'inscrit dans le programme de formation de l'Etablissement.
        </div>
        ...
        <div class="conv-art">
          ...
          Cependant, si l'ENTREPRISE et l'étudiant le conviennent, ce dernier pourra recevoir une indemnité forfaitaire de
          la part de l'ENTREPRISE des frais occasionnés par la mission confiée à l'étudiant.
        </div>
        ...
        <div class="conv-art">
          ...
          <strong>Cependant, l'étudiant est tenu d'informer l'école dans un délai de 24h sur toute modification portant
            sur la convention déjà signée, sinon il en assumera toute sa responsabilité sur son non-respect de la
            convention signée par l'école.</strong><br><br>
          Toutefois, le Stagiaire est soumis à la discipline et au règlement intérieur de L'ENTREPRISE.<br>
          En cas de manquement, L'ENTREPRISE se réserve le droit de mettre fin au stage après en avoir convenu avec le
